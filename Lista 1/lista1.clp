...
(defrule get_prejuízos_sociais
   ?p <- (person (name ?name) (prejuízos_sociais nil))
   =>
   (printout t "Você sente prejuízos_sociais? (yes/no) ")
   (bind ?s (read))
   (modify ?p (prejuízos_sociais ?s)))
...
(defrule found_fobias_específicas
   ...
   (printout t "Os tratamentos recomendados são tcc, terapia_exposicao" crlf))

(defrule check_fobia_social
   ?p <- (person (name ?name) (ansiedade_intensa yes) (coração_acelerado yes) (isolamento_social yes) (suor_excessivo yes))
   =>
...
(defrule check_tag
   ?p <- (person (name ?name) (ansiedade_intensa yes) (coração_acelerado yes) (tremores yes) (dificuldade_concentracao yes) (suor_excessivo yes) (alterações_de_sono_e_apetite yes))
   =>
   (assert (tag ?name)))
...
(defrule check_toc
   ?p <- (person (name ?name) (comportamentos_repetitivos yes) (obsessoes_compulsoes yes) (impulsividade yes) (prejuizos_sociais yes))
   =>
   (assert (toc ?name)))
...
(defrule check_transtorno_explosivo
   ?p <- (person (name ?name) (irritabilidade yes) (impulsividade yes))
   =>
...
(defrule check_borderline
   ?p <- (person (name ?name) (irritabilidade yes) (mudanças_de_humor yes) (pensamentos_suicidas yes) (impulsividade yes) (prejuizos_sociais yes) (autodestrutivo yes))
   =>
...
(defrule check_esquizofrenia
   ?p <- (person (name ?name) (dificuldade_concentracao yes) (mudanças_de_humor yes) (paranoia yes) (prejuizos_sociais yes) (autodestrutivo yes))
   =>
...
(defrule check_tdah
   ?p <- (person (name ?name) (dificuldade_concentracao yes) (impulsividade yes) (prejuizos_sociais yes) (alterações_de_sono_e_apetite yes))
   =>
   (assert (tdah ?name)))
...
(defrule check_depressão
   ?p <- (person (name ?name) (mudanças_de_humor yes) (pensamentos_suicidas yes) (isolamento_social yes) (alterações_de_sono_e_apetite yes) (baixa_energia_e_apatia yes))
   =>
